...
class M_cuti extends CI_Model
{
    ...
    // rekap
    public function get_data_cuti_by_date($start_date, $end_date)
    {
        $this->db->select('cuti.*, user.*, user_detail.*');
        $this->db->from('cuti');
        $this->db->join('user', 'cuti.id_user = user.id_user');
        $this->db->join('user_detail', 'user.id_user_detail = user_detail.id_user_detail');
        $this->db->where('cuti.tgl_diajukan >=', $start_date);
        $this->db->where('cuti.tgl_diajukan <=', $end_date);

...
class Cuti extends CI_Controller
{
	...
	public function view_super_admin()
	{
		if ($this->session->userdata('logged_in') == true and $this->session->userdata('id_user_level') == 3) {
			...
			$data['super_admin_data'] = $this->m_user->get_super_admin_by_id($this->session->userdata('id_user'))->result_array();
	...
	public function batalkan_cuti()
	{
		$id_cuti = $this->input->post("id_cuti");
		$id_user = $this->input->post("id_user");

		// Update status cuti menjadi dibatalkan (misalnya dengan status ID 4)
		$id_status_cuti = 4;

		$hasil = $this->m_cuti->confirm_cuti($id_cuti, $id_status_cuti, null); // Mengosongkan alasan verifikasi
	...
	// rekap
	public function cetak_data_cuti()
	{
		$start_date = $this->input->post('start_date');
		$end_date = $this->input->post('end_date');

		// Ambil data cuti berdasarkan tanggal
		$data['cuti'] = $this->m_cuti->get_data_cuti_by_date($start_date, $end_date);

		// Load view cetak
		$this->load->view('cetak_data_cuti', $data);
	}
